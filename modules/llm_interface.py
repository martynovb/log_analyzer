...
from modules.keyword_models import ExtractedKeyword, KeywordType
...
class LLMInterface(ABC):
    ...
    @abstractmethod
    def extract_keywords(self, issue_description: str) -> List[
        ExtractedKeyword]:
    ...
    def analyze_logs(self, prompt: str) -> str:
        ...
        try:
            ...
            if not response or not isinstance(response, str):
                raise Exception(
                    f"Invalid response from LLM: {type(response)} - {response}")
...
Extract 5-10 most relevant keywords. Be specific and technical."""
    ...
    def _make_llm_request(self, prompt: str) -> str:
        ...
        try:
            self.logger.debug(
                f"Making LLM request to {url} with model: {self.model}")
            self.logger.debug(
            ...
            self.logger.debug(
                f"OpenAI response structure: {list(result.keys())}")
            ...
            if "choices" in result and len(result["choices"]) > 0:
                ...
                if "message" in choice and "content" in choice["message"]:
                    ...
                    self.logger.warning(
                        f"Unexpected choice structure: {choice}")
                    return str(choice)
            else:
                self.logger.warning(
                    f"Unexpected OpenAI response structure: {result}")
                return str(result)
            ...
            try:
                ...
                raise Exception(
                    f"LLM request failed with status {response.status_code}: {error_detail.get('error', {}).get('message', str(error_detail))}")
            except:
                self.logger.error(
    ...
    def _parse_llm_response(self, response: str, original_text: str) -> List[
        ExtractedKeyword]:
