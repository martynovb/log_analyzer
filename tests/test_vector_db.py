...
from modules.vector_db import VectorDb, VectorDbEntry
...
""")
    ...
    def test_search_functionality(self):
        ...
        for result in results:
            self.assertIsInstance(result, VectorDbEntry)
            self.assertIsInstance(result.chunk, str)  # content
            self.assertIsInstance(result.score, (int, float))  # score
    ...
    def test_search_specific_term(self):
        ...
        found_relevant = any("vector" in content or "database" in content
                             for content in [entry.chunk.lower() for entry in results])
